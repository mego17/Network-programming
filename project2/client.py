...
window = tk.Tk()
buttons = []
waiting_for_turn = True  # Variable to keep track of whether it's currently waiting for the other player's turn
...
def create_gui():
    ...
    note_label = tk.Label(window, text="Player 2 (Yellow) 'O'", font=("Helvetica", 12, "bold"))
...
def click(row, column):
    ...
    if game_over or not waiting_for_turn or player_turn != 2:  # Player 2's turn
        return
    waiting_for_turn = False  # Set waiting_for_turn to False to indicate waiting for the other player's turn
...
def reset_game():
    ...
    game_over = False
    player_turn = 1
    waiting_for_turn = True  # Reset waiting_for_turn to True when resetting the game
    for i in range(ROW_COUNT):
        for j in range(COLUMN_COUNT):
